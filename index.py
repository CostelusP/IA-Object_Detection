...
def detect(file):
    ...
    cv2.imwrite("./result/"+ file, cimg)
    cv2.waitKey(0)

class uploadImgHandler(tornado.web.RequestHandler):

    def post(self):
        file = self.request.files["imgFile"][0]
        fh = open(f"img/{file.filename}", "wb")
        fh.write(file.body)
        fh.close()
        detect(file.filename)
        self.redirect(f"http://localhost:8080/result/{file.filename}")
...
class Application(tornado.web.Application):
    def __init__(self):
        handlers = [
            ("/", uploadImgHandler),
            ("/result/(.*)", tornado.web.StaticFileHandler, {'path': 'result'})
        ]
        settings = {
            "static_path": './static'
            }
